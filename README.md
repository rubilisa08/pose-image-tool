...
- 모델은 Stable Diffusion 1.5 + `lllyasviel/sd-controlnet-openpose`를 사용합니다. FLUX.2-klein 4B는 이 시점 기준 diffusers용 공식 ControlNet 포즈 체크포인트가 없고, 대안으로 시도한 FLUX.1-dev는 Colab 무료 T4 GPU에서 메모리 부족(OOM)으로 안정적으로 돌아가지 않아, 무료 GPU에서 실제로 잘 도는 SD1.5 조합으로 바꿨습니다. 자세한 이유는 `pose_tool.ipynb` 상단 설명 셀 참고.
...
2. 셀을 위에서부터 순서대로 실행합니다: 라이브러리 설치 → mediapipe 호환성 패치 → 임포트/GPU 확인 → 참조 사진 업로드 → OpenPose 추출 → ControlNet+Stable Diffusion 1.5 파이프라인 로드 → `generate_pose_image()` 도구 정의 → 기준 이미지 생성 → 2단계 실험(같은 포즈·다른 프롬프트, 같은 프롬프트·다른 포즈). 참조 사진은 셀 실행 중 업로드 창이 뜨면 그때 직접 올립니다 (Hugging Face 로그인은 필요 없습니다).
